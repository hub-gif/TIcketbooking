...
public class Get_Checi {
    ...
    public boolean Get_CheCi(Connection conn, String startStation, String endStation, Checi[] checis, String selectedType) {
        boolean hasTickets = false;  // 结果标志，默认为没有票

        try {
            // 查询 SQL 语句，添加了终点站的条件
            String SQL = "SELECT * FROM checi WHERE StartStation=? AND EndingStation=? AND Type=?";
            PreparedStatement stm = conn.prepareStatement(SQL);
    ...
    public boolean Get_CheCi(Connection conn, String startStation, String endStation, Checi[] checis) {
        boolean hasTickets = false;  // 结果标志，默认为没有票

        try {
            // 查询 SQL 语句，添加了终点站的条件
            String SQL = "SELECT * FROM checi WHERE StartStation=? AND EndingStation=?";
            PreparedStatement stm = conn.prepareStatement(SQL);
            stm.setString(1, startStation);  // 设置始发地
            stm.setString(2, endStation);    // 设置终点站
